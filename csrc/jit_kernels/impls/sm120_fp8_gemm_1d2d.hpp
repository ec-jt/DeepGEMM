...
public:
    ...
    static std::string generate_impl(const Args& args) {
        return fmt::format(R"(
#include <deep_gemm/impls/sm120_fp8_gemm_1d2d.cuh>

using namespace deep_gemm;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&sm120_fp8_gemm_1d2d_impl<
        {},
        {}, {}, {},
        {},
        {}, {}, {},
...
static void sm120_fp8_gemm_1d2d(const torch::Tensor& a, const torch::Tensor& sfa,
    ...
    const auto& config = get_best_config<SM120ArchSpec>(
    ...
    const auto& tensor_map_a = make_tma_a_desc(major_a, a, m, k,
                                               SM120ArchSpec::get_ab_load_block_m(config.multicast_config, config.block_m),
                                               config.block_k,
                                               static_cast<int>(a.stride(get_non_contiguous_dim(major_a))), 1,
                                               config.smem_config.swizzle_a_mode);
    const auto& tensor_map_b = make_tma_b_desc(major_b, b, n, k,
                                               SM120ArchSpec::get_ab_load_block_n(config.multicast_config, config.block_n),
                                               config.block_k,
                                               static_cast<int>(b.stride(get_non_contiguous_dim(major_b))), 1,
                                               config.smem_config.swizzle_b_mode);
    const auto& tensor_map_d = make_tma_cd_desc(d, m, static_cast<int>(d.size(-1)),
                                                SM120ArchSpec::get_cd_store_block_m(config.block_m),
                                                SM120ArchSpec::get_cd_store_block_n(config.block_n),
...
static void sm120_fp8_bmm(const torch::Tensor& a, const torch::Tensor& sfa,
    ...
    DG_HOST_ASSERT(config.smem_config.swizzle_a_mode == config.block_k);
    DG_HOST_ASSERT(config.smem_config.swizzle_b_mode == config.block_k);
    const int& load_block_m = SM120ArchSpec::get_ab_load_block_m(config.multicast_config, config.block_m);
    ...
    const int& load_block_n = SM120ArchSpec::get_ab_load_block_n(config.multicast_config, config.block_n);
    ...
    const int& store_block_m = SM120ArchSpec::get_cd_store_block_m(config.block_m);
    const int& store_block_n = SM120ArchSpec::get_cd_store_block_n(config.block_n);
