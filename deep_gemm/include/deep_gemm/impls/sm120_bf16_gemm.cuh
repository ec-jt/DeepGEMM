...
sm120_bf16_gemm_impl(int* grouped_layout,
                    uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        // SM120: no GMMA descriptors needed (register-based MMA)
        // SM120: descriptor base addresses not needed
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                // SM120: Per-warp BF16 MMA using CuTe SM80 atom
                {
                    using BF16_MMA_Op = cute::SM80_16x8x16_F32BF16BF16F32_TN;
                    auto bf16_tiled_mma = cute::make_tiled_mma(cute::MMA_Atom<BF16_MMA_Op>{});
                    auto bf16_thr_mma = bf16_tiled_mma.get_thread_slice(lane_idx);

                    static constexpr uint32_t BF16_MMA_K = 16;
                    static constexpr uint32_t N_TILES_BF16 = BLOCK_N / 8;

                    #pragma unroll
                    for (uint32_t local_idx = 0; local_idx < BLOCK_M / WAVE_BLOCK_M; ++ local_idx) {
                        auto shifted_accum = accum + WGMMA::kNumAccum * local_idx;

                        // Zero accumulators for this wave
                        #pragma unroll
                        for (uint32_t i = 0; i < WGMMA::kNumAccum; ++ i)
                            shifted_accum[i] = 0.0f;

                        #pragma unroll
                        for (uint32_t n = 0; n < N_TILES_BF16; ++ n) {
                            #pragma unroll
                            for (uint32_t k = 0; k < BLOCK_K / BF16_MMA_K; ++ k) {
                                // A tile: [16, 16] from smem_a
                                auto sA_k = cute::make_tensor(
                                    cute::make_smem_ptr(reinterpret_cast<cute::bfloat16_t*>(
                                        smem_a[stage_idx] + (math_wg_idx * WGMMA::M + local_idx * WAVE_BLOCK_M + (warp_idx % 4) * 16) * BLOCK_K + k * BF16_MMA_K)),
                                    cute::make_layout(cute::make_shape(cute::Int<16>{}, cute::Int<16>{}),
                                                      cute::make_stride(cute::Int<BLOCK_K>{}, cute::Int<1>{})));

                                // B tile: [8, 16] from smem_b
                                auto sB_k = cute::make_tensor(
                                    cute::make_smem_ptr(reinterpret_cast<cute::bfloat16_t*>(
                                        smem_b[stage_idx] + n * 8 * BLOCK_K + k * BF16_MMA_K)),
                                    cute::make_layout(cute::make_shape(cute::Int<8>{}, cute::Int<16>{}),
                                                      cute::make_stride(cute::Int<BLOCK_K>{}, cute::Int<1>{})));

                                auto tAsA = bf16_thr_mma.partition_A(sA_k);
                                auto tBsB = bf16_thr_mma.partition_B(sB_k);
                                auto tArA = bf16_thr_mma.partition_fragment_A(sA_k);
                                auto tBrB = bf16_thr_mma.partition_fragment_B(sB_k);

                                CUTE_UNROLL
                                for (int ii = 0; ii < cute::size(tArA); ++ii)
                                    tArA(ii) = tAsA(ii);
                                CUTE_UNROLL
                                for (int ii = 0; ii < cute::size(tBrB); ++ii)
                                    tBrB(ii) = tBsB(ii);
                                ...
                                float d[4];
                                float c[4] = {shifted_accum[n*4+0], shifted_accum[n*4+1],
                                              shifted_accum[n*4+2], shifted_accum[n*4+3]};
                                ...
                                shifted_accum[n*4+0] = d[0];
                                shifted_accum[n*4+1] = d[1];
                                shifted_accum[n*4+2] = d[2];
                                shifted_accum[n*4+3] = d[3];
                            }
                        }
                    }
                }
