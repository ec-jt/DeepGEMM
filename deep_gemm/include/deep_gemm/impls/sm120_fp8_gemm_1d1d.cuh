...
sm120_fp8_gemm_1d1d_impl(__nv_fp8_e4m3* gmem_a_ptr, __nv_fp8_e4m3* gmem_b_ptr,
...
#if (defined(__CUDA_ARCH__) and (__CUDA_ARCH__ >= 900)) or defined(__CLION_IDE__)  // SM120 uses SM90 TMA + SM89 MMA
    // Scaling checks
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; ++ k_block_idx) {
                ...
                // Each warp handles 16 rows of the BLOCK_M output
                // A[16, BLOCK_K] * B[BLOCK_K, BLOCK_N] -> C[16, BLOCK_N]
                {
                    using MMA_Op = cute::SM89_16x8x32_F32E4M3E4M3F32_TN;
                    auto tiled_mma = cute::make_tiled_mma(cute::MMA_Atom<MMA_Op>{});
                    auto thr_mma = tiled_mma.get_thread_slice(lane_idx);

                    static constexpr uint32_t N_TILES = BLOCK_N / 8;
                    ...
                    for (uint32_t n = 0; n < N_TILES; ++ n) {
                        ...
                        for (uint32_t k = 0; k < BLOCK_K / MMA_K; ++ k) {
                            // A tile: [16, 32] from smem_a at row offset (warp_idx%4)*16, K offset k*32
                            auto sA_k = cute::make_tensor(
                                cute::make_smem_ptr(reinterpret_cast<cute::float_e4m3_t*>(
                                    smem_a[stage_idx] + math_wg_idx * WGMMA::M * BLOCK_K + (warp_idx % 4) * 16 * BLOCK_K + k * MMA_K)),
                                cute::make_layout(cute::make_shape(cute::Int<16>{}, cute::Int<32>{}),
                                                  cute::make_stride(cute::Int<BLOCK_K>{}, cute::Int<1>{})));

                            // B tile: [8, 32] from smem_b at N offset n*8, K offset k*32
                            ...
                            auto tAsA = thr_mma.partition_A(sA_k);
                            auto tBsB = thr_mma.partition_B(sB_k);
                            auto tArA = thr_mma.partition_fragment_A(sA_k);
                            auto tBrB = thr_mma.partition_fragment_B(sB_k);
    ...
    if (blockIdx.x == 0 and threadIdx.x == 0)
        DG_DEVICE_ASSERT(false and "This kernel requires sm_90a or sm_120a");
#endif
}
