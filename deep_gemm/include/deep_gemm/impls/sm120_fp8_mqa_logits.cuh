...
void sm120_fp8_mqa_logits(const uint32_t seq_len, const uint32_t seq_len_kv,
    ...
    // SM120: no register reconfigurations needed
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        while (block_q_idx < num_q_blocks) {
            ...
            for (uint32_t kv_block_idx = 0; kv_block_idx < num_kv_blocks; ++ kv_block_idx) {
                ...
                static constexpr uint32_t N_DIM = BLOCK_Q * kNumHeads;
                        ...
                        for (uint32_t k = 0; k < kHeadDim / 32; ++ k) {
                            auto sKV_k = cute::make_tensor(
                                cute::make_smem_ptr(reinterpret_cast<cute::float_e4m3_t*>(
                                    smem_kv[kv_stage_idx] + warp_offset * kHeadDim + k * 32)),
                            ...
                            auto sQ_k = cute::make_tensor(
                                cute::make_smem_ptr(reinterpret_cast<cute::float_e4m3_t*>(
                                    smem_q[q_stage_idx] + n * 8 * kHeadDim + k * 32)),
