...
sm120_fp8_gemm_1d2d_impl(float* sfb, int* grouped_layout,
                        uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        // SM120: no register reconfig needed
        ...
        // Math warp-groups — SM120 per-warp MMA using SM89 atom

        // NOTES: use `__shfl_sync` to encourage NVCC to use unified registers
        const auto math_wg_idx = __shfl_sync(0xffffffff, threadIdx.x / 128, 0);
        const auto r_0 = warp_idx * 16 + lane_idx / 4, r_1 = r_0 + 8;

        // SM120: no shared memory descriptors needed (using register-based MMA)
        (void)smem_a; (void)smem_b; // suppress unused warnings for desc-related code
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            if (scheduler.is_computation_valid(m_block_idx, math_wg_idx * WGMMA::M)) {
                ...
                dispatch_num_former_iters<0, kGap, kEnd>(kShouldOptimize ? num_former_iters : 0, [&](auto _) {
                    #pragma unroll 8
                    for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                        // SM120: descriptor base addresses not needed (register-based MMA)
                        ...
                        for (uint32_t local_idx = 0; local_idx < BLOCK_M / WAVE_BLOCK_M; ++ local_idx) {
                            ...
                            // SM120: Per-warp MMA using CuTe SM89 atom
                            {
                                using MMA_Op = cute::SM89_16x8x32_F32E4M3E4M3F32_TN;
                                auto tiled_mma_op = cute::make_tiled_mma(cute::MMA_Atom<MMA_Op>{});
                                auto thr_mma_op = tiled_mma_op.get_thread_slice(lane_idx);

                                static constexpr uint32_t N_TILES_MMA = BLOCK_N / 8;
                                static constexpr uint32_t MMA_K = 32;
                                ...
                                #pragma unroll
                                for (uint32_t n = 0; n < N_TILES_MMA; ++ n) {
                                    #pragma unroll
                                    for (uint32_t k = 0; k < BLOCK_K / MMA_K; ++ k) {
                                        auto sA_k = cute::make_tensor(
                                            cute::make_smem_ptr(reinterpret_cast<cute::float_e4m3_t*>(
                                                smem_a[stage_idx] + (math_wg_idx * WGMMA::M + m_offset + (warp_idx % 4) * 16) * BLOCK_K + k * MMA_K)),
                                        ...
                                        auto sB_k = cute::make_tensor(
                                            cute::make_smem_ptr(reinterpret_cast<cute::float_e4m3_t*>(
                                                smem_b[stage_idx] + n * 8 * BLOCK_K + k * MMA_K)),
                                            cute::make_layout(cute::make_shape(cute::Int<8>{}, cute::Int<32>{}),
                                                              cute::make_stride(cute::Int<BLOCK_K>{}, cute::Int<1>{})));

                                        auto tAsA = thr_mma_op.partition_A(sA_k);
                                        auto tBsB = thr_mma_op.partition_B(sB_k);
                                        auto tArA = thr_mma_op.partition_fragment_A(sA_k);
                                        auto tBrB = thr_mma_op.partition_fragment_B(sB_k);
                                        ...
                                        float d[4];
                                        float c[4] = {accum[n*4+0], accum[n*4+1], accum[n*4+2], accum[n*4+3]};
